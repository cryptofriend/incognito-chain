...
func (bestStateBeacon *BestStateBeacon) GetOneShardPendingValidator(shardID byte) []string {
	bestStateBeacon.lockMu.RLock()
	defer bestStateBeacon.lockMu.RUnlock()
	return bestStateBeacon.ShardPendingValidator[shardID]
}

func (bestStateBeacon *BestStateBeacon) GetAllShardPendingValidator() (res map[byte][]string) {
	bestStateBeacon.lockMu.RLock()
	defer bestStateBeacon.lockMu.RUnlock()
	res = make(map[byte][]string)
...
func (bestStateBeacon *BestStateBeacon) GetCurrentShard() byte {
	bestStateBeacon.lockMu.RLock()
	defer bestStateBeacon.lockMu.RUnlock()
	for shardID, isCurrent := range bestStateBeacon.ShardHandle {
		if isCurrent {
			return shardID
		}
	}
